...
| 🚨 **A 429 STILL BURNS THE SESSION — this line used to claim the opposite** | `session_manager.classify()` exists, is unit-tested, and has **ZERO production call sites** (verified 2026-09-01: the only hits are its own definition and a docstring). The live loop at `session_manager.py:167` puts `status_code == 429` straight into `is_blocked` and calls `vault.mark_invalid`, which sets `is_valid=0` **and** `srem`s the profile from `valid_profiles`. So one Etsy throttle retires the profile — and for `etsy_private` that profile is the seller account D-29 calls unreplaceable. **Believe the code, not this table: the fix is in the access layer and is the operator's to make (Rule 6).** Until then, treat every private run as able to cost the account. |
...
| `docs/MCP.md` | **tutorial**: wiring the MCP server (25 tools; seven group 52 operations) into Claude Code/Desktop/Antigravity, the tool table, troubleshooting, and where DeepSeek is allowed to touch the system |
...
| `docs/COST_POLICY.md` | 💰 **which model, how much effort, and when NOT to fan out.** The rule that saves most: a workflow costs ~50-200x doing it directly. If you can predict what the agents will say, you already have the answer. |
| `docs/OPERATOR_FIXES.md` | 🔑 **the two access-layer fixes an agent may not make, written out exactly** — the 429 that still evicts the seller session, and the hardcoded key on an open port. Plus the launch that unblocks LEARN. |
