...
| **🔶 IN PROGRESS: full physical separation from `pinterest-apify`** | The two projects share one Redis (db 0) by convenience, not necessity. `FOREIGN_PROFILE_PREFIXES` (D-47, 2026-08-25) stops their sessions entering our pool, but cannot stop the reverse — confirmed the same day, their own `identities.py::export()` pulls every profile in the shared pool, ours included, to a local file. A filter on our side cannot close that; only a database they have no credentials to can. | Operator-approved 2026-08-25. Plan: a second Redis container + second Go cookie server dedicated to this project, Chrome extension re-pointed at the new one. `pinterest-apify` needs nothing from us — it captures its own sessions via AdsPower — so its setup is untouched. Being executed as a fresh clone into `Desktop\etsy-hunter`, set up manually so the operator understands each piece. |
