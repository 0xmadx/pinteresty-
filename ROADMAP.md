...
| ~~**Full physical separation from `pinterest-apify`**~~ | ✅ **Done 2026-08-25 — the other direction from what was planned.** The plan here was to move THIS project onto a new Redis/Go server; instead `pinterest-apify` moved itself, onto its own `pinterest-redis` container (port 6380). This project's access layer needed zero changes — it never depended on the other side. Their 7 `ads_*` jars, confirmed abandoned (heartbeats stopped advancing) after their cutover, were purged from db 0. See `docs/VAULT_SEPARATION.md`. | The db 0/db 1 mirror (D-33) is kept regardless — cheap, and a safety net if sharing ever resumes. Retiring it entirely is a separate, not-yet-taken decision; see §3 below. |
